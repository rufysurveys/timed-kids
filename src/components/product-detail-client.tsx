...
export function ProductDetailClient({ slug, initialProduct }: { slug: string; initialProduct?: Product }) {
  ...
  return <section className="product-detail"><div className="product-media"><div className="product-detail-image"><Image src={product.image} alt={product.name} fill priority sizes="(max-width: 700px) 100vw, 50vw" /></div>{product.video && <video className="product-video" src={product.video} controls playsInline preload="metadata">Your browser does not support product videos.</video>}</div><div className="product-detail-copy"><span>{product.category}</span><h1>{product.name}</h1><p className="detail-rating"><b>★ {product.rating}</b>{product.reviews ? ` from ${product.reviews} verified reviews` : " New arrival"}</p><div className="detail-price"><strong>{naira.format(product.price)}</strong>{product.oldPrice && <del>{naira.format(product.oldPrice)}</del>}</div><p>{product.description}</p><ul><li>{product.stock ?? 0} currently in stock</li><li>Delivery available across Nigeria</li><li>Seven-day return window</li></ul><ProductActions product={product} /></div></section>;
}
