...
export default function OwnerPage() {
  ...
  async function addProduct(event: FormEvent<HTMLFormElement>) {
    ...
    const formElement = event.currentTarget; const form = new FormData(formElement); const file = form.get("image") as File; const video = form.get("video") as File; const supabase = createClient();
    ...
    if (video?.size) {
      if (video.size > 50 * 1024 * 1024) { setLoading(false); return setMessage("Product videos must be 50 MB or smaller."); }
      const videoPath = `${slug}-${video.name.replace(/[^a-zA-Z0-9.-]/g, "-")}`;
      const videoUpload = await supabase.storage.from("product-images").upload(videoPath, video, { upsert: false });
      if (videoUpload.error) { setLoading(false); return setMessage(videoUpload.error.message); }
      videoUrl = supabase.storage.from("product-images").getPublicUrl(videoPath).data.publicUrl;
    }
    const result = await supabase.from("store_products").insert({ slug, name: String(form.get("name")), category: String(form.get("category")), description: String(form.get("description")), price: Number(form.get("price")), old_price: form.get("oldPrice") ? Number(form.get("oldPrice")) : null, stock: Number(form.get("stock")), image_url: publicImage.publicUrl, video_url: videoUrl, badge: String(form.get("badge") || "") || null }).select().single();
    setLoading(false);
    if (result.error) return setMessage(result.error.message);
